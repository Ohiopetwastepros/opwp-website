...
- Current production version: `403ed092-1cb1-485b-8dd3-af26ba95a70d`
...
- Stripe is the selected initial dog-food processor. D1 remains the system of record; Stripe is used only to tokenize payment methods and execute first-order and future off-session charges.
- When Stripe keys are configured, website checkout creates the normalized order, redirects to Stripe Checkout, charges the first order immediately, and saves the payment method for future monthly charges. Without configured keys, the tool clearly falls back to a saved unpaid request instead of implying that a card was charged.
...
- Existing customers can receive a setup-only Stripe Checkout link from dog-food operations; this saves a payment method without collecting another payment. D1 stores only Stripe identifiers plus card brand/last four, never full card data.
- The audited SNG/CardPointe manual-payment path remains available during transition and for exceptional reconciliation.
...
- A website order may be paid immediately before route selection. A paid monthly order activates its standalone subscription when management confirms the first route date; recurrence is calendar-month based and the next charge is scheduled two days before the next delivery.
- Verified Stripe webhooks are idempotently recorded in `dog_food_payment_events`. Successful payments settle the order, failed payments hold delivery and queue customer notifications, full refunds update the ledger, and duplicate webhook delivery cannot duplicate a recorded payment.
- The daily renewal worker creates one unique order per subscription/delivery cycle and submits an off-session Stripe PaymentIntent. It cannot run until production Stripe secrets are configured.
...
2. Create the OPWP Stripe account/test workspace, install `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET`, register the production webhook, and complete a Stripe test-mode first-order, setup-only, failure, and refund run before enabling live mode.
