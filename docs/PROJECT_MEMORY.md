...
- Current production version: `403ed092-1cb1-485b-8dd3-af26ba95a70d` (deployed 2026-07-20)
...
- Added guarded internal order entry and auditable manual payment confirmation. The manual SNG/CardPointe path remains an exception and transition fallback rather than the owner of food recurrence.
- Stripe is the selected initial processor. The implementation uses immediate Stripe Checkout for first orders, setup-only Checkout for already-paid customers, signed/idempotent webhooks, stored Stripe identifiers rather than card data, and D1-owned calendar-month renewal orders charged off-session two days before delivery.
- A paid website monthly order may wait for route selection; confirming its first delivery date activates the native calendar-month subscription. Duplicate payment references, duplicate captured payments, and duplicate billing-cycle orders are rejected.
- Jim Hansen order `EDF-JH-20260720-01` is delivered, fulfilled, and paid for one Red 30-20 and one Blue 26-14 bag: $118.00 subtotal, $9.14 tax, and $127.14 total.
...
- SNG payment acceptance now automatically settles a dog-food order only when SNG client ID, exact cents, a unique recent dog-food-only finalized invoice, and a unique unpaid order all agree. This avoids both manual polling and name-based false matches while retaining the SNG transaction reference for idempotency and audit.
- Jim Hansen's SNG payment reference `201047048403` and finalized invoice `3559780` match order `EDF-JH-20260720-01` exactly at $127.14. D1 also received two SNG `subscription_created` events during the charge flow; those external subscriptions must not remain the future source of recurrence once Jim is moved to the standalone system.
- Stripe code is safe to deploy without keys because the public tool detects configuration and retains the unpaid-request fallback. Live/test charging remains disabled until `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` are installed and the Stripe webhook is registered.
