...
import stripeStyles from "./stripe-controls.module.css";
...
export default function DogFoodOperationsClient({ initialData }) {
  ...
  async function createStripeSetup(customer) {
    setBusy(`stripe-${customer.id}`); setError("");
    try {
      const result = await operation({ action: "create_stripe_setup", customerId: customer.id });
      try {
        await navigator.clipboard.writeText(result.checkoutUrl);
        setNotice(`Secure Stripe setup link copied for ${customer.first_name} ${customer.last_name}. Send this link only to that customer.`);
      } catch {
        window.prompt(`Copy the secure Stripe setup link for ${customer.first_name} ${customer.last_name}:`, result.checkoutUrl);
      }
  ...
  return <>
    ...
    {view === "orders" ? <section className={styles.panel}><header><div><span className={styles.eyebrow}>Commerce queue</span><h2>Orders and fulfillment</h2></div><p>Unpaid orders can be corrected before payment is recorded.</p></header><div className={styles.tableWrap}><table><thead><tr><th>Order</th><th>Customer</th><th>Items</th><th>Delivery</th><th>Total</th><th>Payment</th><th>Action</th></tr></thead><tbody>{initialData.orders.length ? initialData.orders.map((order) => <tr key={order.id}><td><strong>{order.order_number}</strong><small>{order.order_type === "subscription" ? "Monthly" : "One time"}</small></td><td><strong>{order.first_name} {order.last_name}</strong><small>{String(order.customer_type).replaceAll("_", " ")}{order.sng_client_id ? " · SNG linked" : ""}</small></td><td className={styles.items}>{order.items || "No items"}</td><td><strong>{shortDate(order.scheduled_date)}</strong><small>{order.delivery_status ? <Status value={order.delivery_status} /> : "Date confirmation needed"}</small></td><td><strong>{money(order.total_cents)}</strong><small>{order.delivery_fee_cents ? `${money(order.delivery_fee_cents)} delivery` : "Free route delivery"}</small></td><td><Status value={order.status} />{order.payment_provider ? <small>{String(order.payment_provider).replaceAll("_", " ")}</small> : null}</td><td><div className={styles.rowActions}>{["draft","pending_payment","payment_failed"].includes(order.status) ? <button className={styles.editAction} type="button" onClick={() => { setEditOrder(order); setError(""); }}>Edit order</button> : null}{!order.scheduled_date ? <button className={styles.tableAction} type="button" onClick={() => { setScheduleOrder(order); setError(""); }}>Set delivery date</button> : ["pending_payment","payment_failed"].includes(order.status) ? <button className={styles.tableAction} type="button" onClick={() => { setPaymentOrder(order); setError(""); }}>Confirm payment</button> : <span className={styles.completeMark}>✓ Recorded</span>}</div></td></tr>) : <tr><td colSpan="7" className={styles.empty}>No dog-food orders have been entered.</td></tr>}</tbody></table></div></section> : null}

    {view === "subscriptions" ? <section className={styles.panel}><header><div><span className={styles.eyebrow}>Standalone recurring engine</span><h2>Monthly subscriptions</h2></div><p>Subscriptions activate after the first payment is confirmed and a route date is set.</p></header><div className={styles.cards}>{initialData.subscriptions.length ? initialData.subscriptions.map((subscription) => <article className={styles.subscription} key={subscription.id}><div><span>{subscription.first_name?.[0]}{subscription.last_name?.[0]}</span><div><strong>{subscription.first_name} {subscription.last_name}</strong><small>{subscription.items || "No active products"}</small></div></div><dl><div><dt>Frequency</dt><dd>Monthly</dd></div><div><dt>Next charge</dt><dd>{shortDate(subscription.next_charge_at)}</dd></div><div><dt>Next delivery</dt><dd>{shortDate(subscription.next_delivery_date)}</dd></div></dl><footer><Status value={subscription.status} /><small>{subscription.card_brand || String(subscription.payment_provider || "Payment method needed").replaceAll("_", " ")}{subscription.card_last_four ? ` •••• ${subscription.card_last_four}` : ""}</small></footer></article>) : <div className={styles.emptyCard}><strong>No active subscriptions yet</strong><p>Create a monthly order and confirm its first payment to activate the recurring schedule here.</p></div>}</div></section> : null}
    ...
    {view === "customers" ? <section className={styles.panel}><header><div><span className={styles.eyebrow}>Unified customer record</span><h2>Dog-food customers</h2></div><p>SNG IDs link scooping customers without moving food subscriptions into SNG.</p></header><div className={styles.customerGrid}>{initialData.customers.map((customer) => <article key={customer.id}><span>{customer.first_name?.[0]}{customer.last_name?.[0]}</span><div><strong>{customer.first_name} {customer.last_name}</strong><p>{customer.email}</p><small>{customer.city || "Address needed"}{customer.route_day ? ` · ${customer.route_day} route` : ""}</small></div><div><Status value={customer.customer_type} />{customer.stripe_customer_id ? <small>{customer.card_brand ? `${customer.card_brand} •••• ${customer.card_last_four}` : "Stripe profile ready"}</small> : customer.sng_client_id ? <small>SNG linked</small> : <small>Native customer</small>}{initialData.stripeConfigured && !customer.card_last_four ? <button className={stripeStyles.setupButton} type="button" disabled={busy === `stripe-${customer.id}`} onClick={() => createStripeSetup(customer)}>{busy === `stripe-${customer.id}` ? "Creating…" : "Copy payment setup link"}</button> : null}</div></article>)}</div></section> : null}
