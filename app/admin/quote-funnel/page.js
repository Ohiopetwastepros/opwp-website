...
export default async function QuoteFunnelPage() {
  ...
  return <main style={{ maxWidth: 1320, margin: "0 auto", padding: "34px 22px 70px", fontFamily: "system-ui,sans-serif", color: "#17384f" }}>
    ...
    <div style={{ overflowX: "auto", border: "1px solid #dce4e6", borderRadius: 12, background: "#fff" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr>{["Last activity","Stage","Contact","ZIP","SNG","Owner email"].map((label) => <th key={label} style={{ ...cell, color: "#5a6b75", fontSize: 11, textTransform: "uppercase" }}>{label}</th>)}</tr></thead><tbody>
      {rows.length ? rows.map((row) => <tr key={row.id}><td style={cell}>{row.last_activity_at || row.created_at}</td><td style={cell}><strong>{row.kind}</strong><br /><small>{row.lifecycle_stage || row.status}</small></td><td style={cell}>{row.name || "Name pending"}<br />{row.email || "—"}<br />{row.phone || "—"}</td><td style={cell}>{row.zip || "—"}</td><td style={cell}>{row.sng_sync_state || "not attempted"}{row.sng_entity_id ? <><br /><small>ID {row.sng_entity_id}</small></> : null}</td><td style={cell}>{row.notifications || "not queued"}</td></tr>) : <tr><td style={cell} colSpan={6}>No website funnel activity yet.</td></tr>}
    </tbody></table></div>
  </main>;
}
