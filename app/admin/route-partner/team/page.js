...
export default async function FieldTeamPage() {
  ...
  return <main className={`opwp-admin-shell ${styles.page}`}>
    <header className={styles.topbar}><a href="/admin/route-partner/" className={styles.brand}><span>O</span><strong>OPWP</strong><small>Field operations</small></a><nav><a href="/admin/route-partner/">Route control</a><a className={styles.active} href="/admin/route-partner/team/">Field team</a><a href="/field/">Technician view</a></nav><div className={styles.identity}><span>{auth.email}</span><SignOutButton /></div></header>
