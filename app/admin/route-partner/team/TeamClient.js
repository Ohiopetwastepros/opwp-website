...
export default function TeamClient() {
  ...
  return <>
    ...
    {formOpen ? <div className={styles.modalBackdrop} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setFormOpen(false); }}><form className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="add-technician-title" onSubmit={saveMember}><span>New field account</span><h2 id="add-technician-title">Add a technician</h2><p>The CRM employee ID must match the technician ID on imported routes.</p><label>Full name<input name="name" required autoFocus /></label><label>Email<input name="email" type="email" autoComplete="email" required /></label><label>CRM employee ID<input name="externalId" required /></label><label>Six-digit PIN<input name="pin" inputMode="numeric" pattern="[0-9]{6}" maxLength="6" autoComplete="new-password" required /></label><div><button type="button" onClick={() => setFormOpen(false)}>Cancel</button><button type="submit" disabled={Boolean(loading)}>Create access</button></div></form></div> : null}
  </>;
}
