...
import { authenticateOfficeMember, createFieldSession, expiredOfficeSessionCookie, fieldAuthConfigured, officeSessionCookie, revokeOfficeSession, verifyOfficeRequest } from "@/lib/field-auth";
...
export async function POST(request) {
  const rate = await checkRateLimit(request, { scope: "office_login", limit: 20, windowSeconds: 900, failClosed: true });
  if (!rate.allowed) return rateLimitResponse(rate);
  const db = getDb();
  if (!db || !fieldAuthConfigured()) return NextResponse.json({ ok: false, error: "Office access is not configured." }, { status: 503 });
