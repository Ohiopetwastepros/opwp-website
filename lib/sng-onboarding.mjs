...
export function publicOnboardingFailure(upstream = {}) {
  if (!upstream.configured) {
    return {
      code: "integration_unavailable",
      message: "Account setup is temporarily unavailable. Nothing was charged; please contact us.",
    };
  }
  const providerMessage = [upstream.data?.error, upstream.data?.message]
    .filter((value) => typeof value === "string")
    .join(" ");
  if (/client.*email.*already.*onboarded|already.*onboarded/i.test(providerMessage)) {
    return {
      code: "account_exists",
  ...
  return {
    ...
    message: "Sweep & Go could not create the account. Nothing was charged; please retry or contact us.",
  };
}
