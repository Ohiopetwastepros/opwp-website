...
export function validateOnboardingInput(body) {
  ...
  if (!selectionsValid || !value.funnel_id || !value.first_name || !value.last_name || !value.email || !value.cell_phone_number || !value.home_address || !value.city || !["OH", "MI"].includes(value.state) || !value.zip_code || !Number.isInteger(value.number_of_dogs) || value.number_of_dogs < 1 || value.number_of_dogs > 7 || ![0, 1].includes(value.initial_cleanup_required) || ![0, 1].includes(value.marketing_allowed) || value.terms_open_api !== 1) {
