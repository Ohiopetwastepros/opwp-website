...
import { normalizeHowHeard, repeatDogValue, selectedSngCrossSells } from "./sng-onboarding.mjs";
...
export async function sngRequest(path, { method = "GET", body, searchParams } = {}) {
  ...
  try {
    const response = await outboundFetch(url, {
      ...
      cache: "no-store",
    }, { provider: "sweep_and_go", operation: path, timeoutMs: 12000, retryable: method === "GET", retries: method === "GET" ? 1 : 0 });
    const data = await response.json().catch(() => ({}));
    return { configured: true, ok: response.ok, status: response.status, data };
  } catch (error) {
    console.error(JSON.stringify({ event: "outbound_request_failed", provider: "sweep_and_go", operation: path, classification: "request_failed" }));
    return { configured: true, ok: false, status: 0, data: { error: "Sweep & Go could not be reached." } };
  }
}
...
export function toOnboardingPayload(input) {
  const dogCount = Number(input.number_of_dogs);
  const crossSells = selectedSngCrossSells(input.selected_addons);
  return {
    ...
    how_heard_about_us: normalizeHowHeard(input.tracking_field),
    additional_comment: input.account_note,
    dog_name: repeatDogValue(input["dog_name[]"] ?? input.dog_name, dogCount, { firstOnly: true }),
    safe_dog: repeatDogValue(input["safe_dog[]"] ?? input.safe_dog, dogCount),
    dog_comment: repeatDogValue(input["dog_comment[]"] ?? input.dog_comment, dogCount, { firstOnly: true }),
    cross_sells: crossSells.ids,
    cross_sells_names: crossSells.names || undefined,
    ...
    gated_community: input.gated_community ?? input.gate_code,
    gate_code: input.gate_code,
    doggie_door: input.doggie_door ?? input.has_doggie_door,
    garbage_can_location: input.garbage_can_location,
    tracking_field: "website_instant_quote",
