...
import { deriveQuickBooksPricingActuals } from "./quickbooks-pricing.mjs";
...
async function qboFetch(record, token, path, params = {}) {
  ...
  const response = await outboundFetch(url, { headers: { Authorization: `Bearer ${token}`, Accept: "application/json" }, cache: "no-store" }, { provider: "quickbooks", operation: path, timeoutMs: 15000, retryable: true, retries: 1 });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
...
export async function withQuickBooksReadOnlyClient(callback) {
  if (!quickBooksConfigured()) throw new Error("QuickBooks OAuth is not configured.");
  const record = await connection();
  if (!record) throw new Error("QuickBooks is not connected.");
  let token = await validAccessToken(record);
  const get = async (path, params = {}) => {
    const result = await qboFetchWithAuthRecovery(record, token, path, params);
    token = result.token;
    return result.data;
  };
  return callback({
    get,
    companyName: record.company_name || "QuickBooks company",
...
export async function getQuickBooksPricingInputs({ tonyWeeklyHours = 40 } = {}) {
  if (!quickBooksConfigured()) return { configured: false, connected: false, ok: false };
  const record = await connection();
  if (!record) return { configured: true, connected: false, ok: false };
  ...
  try {
    const result = await withQuickBooksReadOnlyClient(async ({ get, companyName }) => {
      const report = await get("reports/ProfitAndLoss", {
        start_date: periodStart,
        end_date: periodEnd,
        accounting_method: "Accrual",
      });
      const actuals = deriveQuickBooksPricingActuals(report, { periodStart, periodEnd, tonyWeeklyHours });
      if (!(actuals.directPayroll > 0) || !Number.isFinite(actuals.burdenPercent)) {
        throw new Error("QuickBooks payroll expense detail is unavailable for the pricing period.");
      }
      return { companyName, ...actuals };
    });
    return { configured: true, connected: true, ok: true, periodStart, periodEnd, accountingMethod: "Accrual", ...result };
  } catch (error) {
    const message = String(error);
    console.error(JSON.stringify({ event: "quickbooks_pricing_inputs_error", message }));
    return { configured: true, connected: true, ok: false, periodStart, periodEnd };
  }
}
...
export async function disconnectQuickBooks() {
  ...
  try {
    const refreshToken = await decrypt(record.refresh_token_encrypted);
    const response = await outboundFetch(REVOKE_URL, {
      method: "POST",
      headers: { Authorization: basicAuthorization(), Accept: "application/json", "Content-Type": "application/json" },
      body: JSON.stringify({ token: refreshToken }),
    }, { provider: "quickbooks", operation: "oauth_revoke", timeoutMs: 12000 });
...
function pickOptional(map, ...labels) {
  for (const label of labels) if (map.has(label.toLowerCase())) return map.get(label.toLowerCase());
  return null;
}
...
export async function getQuickBooksFinancialSnapshot() {
  ...
  try {
    ...
    const grossProfit = pickOptional(pnl, "Gross Profit");
    ...
    const snapshot = {
      ...
      revenue, grossProfit, costOfGoodsSold, grossMargin: revenue && grossProfit !== null ? Math.round((grossProfit / revenue) * 1000) / 10 : null,
      ...
      currentAssets: pickOptional(balance, "Total Current Assets"), currentLiabilities: pickOptional(balance, "Total Current Liabilities"),
...
function reportTimeSeries(report) {
  const labels = monthLabels(report);
  const revenue = reportSeries(report, "Total Income", "Income");
  const expenses = reportSeries(report, "Total Expenses", "Expenses");
  const grossProfit = reportSeries(report, "Gross Profit");
  const netIncome = reportSeries(report, "Net Income");
  return labels.map((label, index) => ({
    label,
    revenue: revenue[index] || 0,
    expenses: expenses[index] || 0,
    grossProfit: grossProfit[index] || 0,
    netIncome: netIncome[index] || 0,
    partial: /\b1-\d{1,2}\b/.test(label),
  })).filter((row) => !/^total$/i.test(String(row.label).trim()));
}

function quarterlySeries(monthly = []) {
  const monthNumbers = new Map([["jan",1],["feb",2],["mar",3],["apr",4],["may",5],["jun",6],["jul",7],["aug",8],["sep",9],["oct",10],["nov",11],["dec",12]]);
  const groups = new Map();
  for (const row of monthly) {
    const label = String(row.label).toLowerCase();
    const monthName = label.match(/^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)/)?.[1];
    const year = label.match(/20\d{2}/)?.[0];
    if (!monthName || !year) continue;
    const month = Number(monthNumbers.get(monthName));
    const key = `Q${Math.ceil(month / 3)} ${year}`;
    const current = groups.get(key) || { label: key, revenue: 0, expenses: 0, grossProfit: 0, netIncome: 0, partial: false };
    for (const field of ["revenue", "expenses", "grossProfit", "netIncome"]) current[field] += Number(row[field]) || 0;
    current.partial ||= Boolean(row.partial);
    groups.set(key, current);
  }
  return [...groups.values()];
}

function yearlySeries(monthly = []) {
  const groups = new Map();
  for (const row of monthly) {
    const year = String(row.label).match(/20\d{2}/)?.[0];
    if (!year) continue;
    const current = groups.get(year) || { label: year, revenue: 0, expenses: 0, grossProfit: 0, netIncome: 0, partial: true };
    for (const field of ["revenue", "expenses", "grossProfit", "netIncome"]) current[field] += Number(row[field]) || 0;
...
export async function getQuickBooksFinancialDashboard() {
  ...
  try {
    ...
    const halfYearStart = now.getUTCMonth() < 6 ? yearStart : `${now.getUTCFullYear()}-07-01`;
    const priorHalfEnd = `${now.getUTCFullYear()}-06-30`;
    const monthlyRequests = now.getUTCMonth() < 6
      ...
      : [
        qboFetch(record, token, "reports/ProfitAndLoss", { start_date: yearStart, end_date: priorHalfEnd, accounting_method: "Accrual", summarize_column_by: "Month" }),
        qboFetch(record, token, "reports/ProfitAndLoss", { start_date: halfYearStart, end_date: today, accounting_method: "Accrual", summarize_column_by: "Month" }),
      ];
    ...
    const [monthlyReports, dailyResult, weeklyResult, receivablesResult, payablesResult, classesResult, departmentsResult, accountsResult, itemsResult] = await Promise.all([
      Promise.all(monthlyRequests),
      optional("Daily performance", qboFetch(record, token, "reports/ProfitAndLoss", { start_date: daysBefore(now, 30), end_date: today, accounting_method: "Accrual", summarize_column_by: "Day" }), {}),
      optional("Weekly performance", qboFetch(record, token, "reports/ProfitAndLoss", { start_date: daysBefore(now, 181), end_date: today, accounting_method: "Accrual", summarize_column_by: "Week" }), {}),
    ...
    const monthly = monthlyReports.flatMap(reportTimeSeries);
    const periods = {
      daily: reportTimeSeries(dailyResult.value),
      weekly: reportTimeSeries(weeklyResult.value),
      monthly,
      quarterly: quarterlySeries(monthly),
      yearly: yearlySeries(monthly),
    };
    const activeAccounts = queryRows(accounts, "Account").filter((account) => account.Active !== false);
    const dashboard = {
      ...summary,
      warnings: [dailyResult, weeklyResult, receivablesResult, payablesResult, classesResult, departmentsResult, accountsResult, itemsResult].map((result) => result.warning).filter(Boolean),
      monthly,
      periods,
