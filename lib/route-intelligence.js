...
export async function calculateActiveSubscriptionPlan(db) {
  ...
  const densityInput = selectiveDensityVisits(visits, recommendations, transitionMetrics);
  const densityTransitionVisits = officeTransitionVisits(densityInput.visits);
  const densityMetrics = densityInput.moves.length ? (await calculateRouteMetrics(densityTransitionVisits, { optimize: true })).map(activeMetric) : transitionMetrics;
  const payload = {
    routeModelVersion: 5,
    sourceSnapshotAt: snapshot.capturedAt,
    ...
    selectiveDayScenario: {
      ...
      baselineTotals: metricTotals(transitionMetrics),
      modeledTotals: metricTotals(densityMetrics),
...
export async function runScheduledRouteBookRefresh(env) {
  ...
  try {
    ...
    if (geocode.remaining === 0 && geocode.attempted === 0) {
      ...
      if (String(latest?.source_snapshot_at || "") !== String(geocode.sourceSnapshotAt) || latestWeek !== currentWeek || latestModelVersion !== 5) plan = await calculateActiveSubscriptionPlan(env.DB);
    }
