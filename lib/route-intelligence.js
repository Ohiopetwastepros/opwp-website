...
export async function calculateActiveSubscriptionPlan(db) {
  ...
  const status = stops.every((stop) => stop.location) && metrics.every((metric) => metric.routeStatus === "complete") && transitionMetrics.every((metric) => metric.routeStatus === "complete") ? "complete" : "partial";
...
export async function runScheduledRouteBookRefresh(env) {
  ...
  try {
    ...
    if (geocode.remaining === 0 && geocode.attempted === 0) {
      ...
      if (String(latest?.source_snapshot_at || "") !== String(geocode.sourceSnapshotAt) || latestWeek !== currentWeek || latestModelVersion !== 4) plan = await calculateActiveSubscriptionPlan(env.DB);
    }
