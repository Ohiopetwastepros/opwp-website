...
export async function getSystemHealth() {
  ...
  const notificationSql = "SELECT (SELECT COUNT(*) FROM dog_food_notifications WHERE status='queued') + (SELECT COUNT(*) FROM dog_food_follow_ups WHERE status='queued') + (SELECT COUNT(*) FROM route_partner_notification_outbox WHERE status='queued') queued, (SELECT COUNT(*) FROM dog_food_notifications WHERE status='sending') + (SELECT COUNT(*) FROM dog_food_follow_ups WHERE status='sending') + (SELECT COUNT(*) FROM route_partner_notification_outbox WHERE status='sending') sending, (SELECT COUNT(*) FROM dog_food_notifications WHERE status='sent') + (SELECT COUNT(*) FROM dog_food_follow_ups WHERE status='sent') + (SELECT COUNT(*) FROM route_partner_notification_outbox WHERE status='sent') sent, (SELECT COUNT(*) FROM dog_food_notifications WHERE status='failed') + (SELECT COUNT(*) FROM dog_food_follow_ups WHERE status='failed') + (SELECT COUNT(*) FROM route_partner_notification_outbox WHERE status='failed') failed, MIN(oldest) oldest FROM (SELECT MIN(scheduled_at) oldest FROM dog_food_notifications WHERE status IN ('queued','sending') UNION ALL SELECT MIN(scheduled_at) FROM dog_food_follow_ups WHERE status IN ('queued','sending') UNION ALL SELECT MIN(created_at) FROM route_partner_notification_outbox WHERE status IN ('queued','sending'))";
  const [sng, airtable, stripe, quickbooks, notificationCounts, migration, proofs, syncRuns] = await Promise.all([
