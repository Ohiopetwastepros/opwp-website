...
export async function deliverSubmissionNotification(id, env = getRuntimeEnv()) {
  ...
  try {
    ...
    return { configured: true, status: "sent", providerMessageId: result.messageId };
  } catch (error) {
...
export async function flushSubmissionNotifications(env, limit = 10) {
  const db = env?.DB;
  if (!db || !emailConfig(env).configured) return { configured: false, attempted: 0, sent: 0, failed: 0 };
