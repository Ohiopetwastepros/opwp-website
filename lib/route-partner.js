...
function sourceCompletion(row) {
  const status = text(row.status || row.job_status || row.completion_status || row.state).toLowerCase();
  const completedAt = row.completed_at || row.completedAt || row.finished_at || row.actual_end_time;
  return Boolean(completedAt || row.completed === true || row.is_completed === true || /complete|completed|done|finished/.test(status));
}

export async function validateRoutePartnerCompletions({ db, date, sourceRows, actor, organizationId = OPWP_ORGANIZATION_ID }) {
  if (!validDate(date)) throw new Error("A valid service date is required.");
  const rows = new Map((Array.isArray(sourceRows) ? sourceRows : []).map((row) => [text(row.id || row.job_id || row.uuid), row]));
  const pending = await db.prepare(
    `SELECT t.id,t.external_task_id,t.route_plan_id FROM route_partner_tasks t JOIN route_partner_route_plans p ON p.id=t.route_plan_id
     WHERE t.organization_id=? AND p.service_date=? AND t.task_type='scoop' AND t.crm_completion_status IN ('pending','mismatch')`
  ...
  for (const task of pending.results || []) {
    const source = rows.get(text(task.external_task_id));
    const status = source ? (sourceCompletion(source) ? "validated" : "pending") : "mismatch";
    if (status === "validated") validated += 1;
    else if (status === "pending") pendingCount += 1;
    else mismatches += 1;
    statements.push(db.prepare("UPDATE route_partner_tasks SET crm_completion_status=?,status=CASE WHEN ?='validated' AND status='validation_pending' THEN 'completed' ELSE status END,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(status, status, task.id));
  }
  if (statements.length) await db.batch(statements);
  await db.prepare(
    `INSERT INTO route_partner_plan_events (id,organization_id,event_type,actor_email,details)
     VALUES (?,?, 'crm_completion_validation',?,?)`
  ).bind(crypto.randomUUID(), organizationId, actor, JSON.stringify({ date, validated, pending: pendingCount, mismatches })).run();
  return { checked: statements.length, validated, pending: pendingCount, mismatches };
}
...
export async function getRoutePartnerDay(db, date, organizationId = OPWP_ORGANIZATION_ID) {
  ...
  const [locationResult, taskResult, shiftResult, exceptionResult] = await Promise.all([
    db.prepare(
    ...
    db.prepare(
      `SELECT t.id, t.route_plan_id, t.location_id, t.task_type, t.source_provider, t.external_task_id,
        t.customer_display_name, t.status, t.estimated_minutes, t.placement_note, t.product_summary, t.crm_completion_status,
        (SELECT COUNT(*) FROM route_partner_field_proofs fp WHERE fp.task_id=t.id) AS proof_count,
        (SELECT fp.id FROM route_partner_field_proofs fp WHERE fp.task_id=t.id ORDER BY fp.created_at DESC LIMIT 1) AS latest_proof_id
       FROM route_partner_tasks t WHERE t.route_plan_id IN (${placeholders}) ORDER BY t.location_id, t.task_type`
    ).bind(...planIds).all(),
    db.prepare(
      `SELECT s.route_plan_id,s.id,s.status,s.technician_member_id,s.starting_mileage,s.ending_mileage,s.departed_at,s.completed_at,
        lc.status AS load_status,lc.payment_validation_status
       FROM route_partner_field_shifts s LEFT JOIN route_partner_load_checks lc ON lc.route_plan_id=s.route_plan_id
       WHERE s.route_plan_id IN (${placeholders})`
    ).bind(...planIds).all(),
    db.prepare(
      `SELECT id,route_plan_id,task_id,request_type,reason_code,status,inventory_disposition,requested_by,denial_reason,created_at
       FROM route_partner_change_requests WHERE route_plan_id IN (${placeholders}) ORDER BY created_at DESC`
    ).bind(...planIds).all(),
  ]);
  ...
  for (const task of taskResult.results ?? []) {
    ...
    tasksByLocation.get(task.location_id).push({
      ...
      proofCount: Number(task.proof_count) || 0,
      latestProofId: task.latest_proof_id || null,
    });
  }
  ...
  const shiftsByPlan = new Map((shiftResult.results || []).map((shift) => [shift.route_plan_id, { id: shift.id, status: shift.status, technicianMemberId: shift.technician_member_id, loadStatus: shift.load_status || "pending", paymentStatus: shift.payment_validation_status || "pending", startingMileage: shift.starting_mileage, endingMileage: shift.ending_mileage, departedAt: shift.departed_at, completedAt: shift.completed_at }]));
  const exceptionsByPlan = new Map();
  for (const exception of exceptionResult.results || []) { if (!exceptionsByPlan.has(exception.route_plan_id)) exceptionsByPlan.set(exception.route_plan_id, []); exceptionsByPlan.get(exception.route_plan_id).push({ id: exception.id, taskId: exception.task_id, type: exception.request_type, reason: exception.reason_code, status: exception.status, inventoryDisposition: exception.inventory_disposition, requestedBy: exception.requested_by, denialReason: exception.denial_reason, createdAt: exception.created_at }); }
  const publicPlans = plans.map((plan) => ({
    ...
    field: shiftsByPlan.get(plan.id) || null,
    exceptions: exceptionsByPlan.get(plan.id) || [],
