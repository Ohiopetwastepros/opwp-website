...
export function stripeConfigured(env = getRuntimeEnv()) {
  ...
  return config.secretKey.startsWith("sk_") && config.webhookSecret.startsWith("whsec_");
}
...
async function stripeRequest(path, { method = "POST", form, idempotencyKey, env = getRuntimeEnv() } = {}) {
  ...
  const response = await outboundFetch(`${STRIPE_API}${path}`, { method, headers, body }, {
    provider: "stripe",
    operation: path,
    timeoutMs: 15000,
    retryable: method === "GET" || Boolean(idempotencyKey),
    retries: 1,
  });
