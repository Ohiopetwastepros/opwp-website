...
import { loadAirtableTableSet } from "./airtable-refresh-policy.mjs";
import { airtableFrequency as customerFrequency, sngScheduleValues } from "./sng-service-schedule.mjs";
...
const TABLES = {
  opwp: {
    ...
    reviewTracker: "tblzRg9DZKdIRQDrn",
  },
  dogFood: {
...
async function mappedEmployeeName(employeeId) {
  const db = runtimeEnv().DB;
  if (!db || employeeId === undefined || employeeId === null || employeeId === "") return "";
  try {
    const row = await db.prepare("SELECT display_name FROM staff_integration_mappings WHERE organization_id='org-opwp' AND sng_employee_id=? LIMIT 1").bind(String(employeeId)).first();
...
async function airtableRequest(path, key, attempt = 0) {
  const response = await outboundFetch(`${API_URL}/${path}`, {
    headers: { Authorization: `Bearer ${key}`, Accept: "application/json" },
    next: { revalidate: 300 },
  }, { provider: "airtable", operation: "list_records", timeoutMs: 12000 });
...
export async function updateGoogleReviewTrackingRecord(recordId, fields) {
  const key = runtimeEnv().AIRTABLE_API_KEY;
  if (!key) throw new Error("AIRTABLE_API_KEY is not configured.");
  const response = await fetch(`${API_URL}/${AIRTABLE_BASES.opwp.id}/${TABLES.opwp.reviewTracker}`, {
    method: "PATCH",
    headers: { Authorization: `Bearer ${key}`, Accept: "application/json", "Content-Type": "application/json" },
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data?.error?.message || data?.error?.type || `Airtable review update failed (${response.status})`);
...
export async function upsertSngShift(body, context = {}) {
  ...
  const employeeName = data.employee_name || prior.name || await mappedEmployeeName(data.employee_id);
  const fields = {
    "Record ID": String(shiftId),
    Date: String(data.shift_date || data.start_time || "").slice(0, 10),
    "Tech Name": employeeName,
...
export function classifySngSubscriptionPlan(value) {
  ...
  if (/saniti[sz]|deodori[sz]|haulaway|frontyard/.test(plan)) return "addon";
  if (/regularplan/.test(plan) || /^\d+d[-_/]?(?:\d+x?)?(?:bw|w|m)$/.test(plan) || /^\d+w-\d+d$/.test(plan)) return "scooping";
  return "addon";
}
...
export async function updateSngCustomerLifecycle(body, { status = "", createIfMissing = false } = {}) {
  ...
  const fields = {};
  const hasServiceDays = Object.prototype.hasOwnProperty.call(data, "service_days");
  const hasAssignedTech = Object.prototype.hasOwnProperty.call(data, "assigned_to");
  const schedule = sngScheduleValues({
    ...data,
    cleanup_frequency: data.cleanup_frequency || data.subscription_name,
    service_days: hasServiceDays ? data.service_days : [current["Service Day"], current["Service Day 2"]],
  });
  ...
  if (hasServiceDays && String(current["Service Day"] ?? "") !== String(schedule.days[0] ?? "")) fields["Service Day"] = schedule.days[0] || null;
  if (hasServiceDays && String(current["Service Day 2"] ?? "") !== String(schedule.days[1] ?? "")) fields["Service Day 2"] = schedule.days[1] || null;
  if (sngId && String(current["SNG Client ID"] ?? "") !== String(sngId)) fields["SNG Client ID"] = String(sngId);
  if (hasAssignedTech && String(current["Assigned Tech"] ?? "") !== schedule.assignedTech) fields["Assigned Tech"] = schedule.assignedTech || null;
  if (hasAssignedTech && String(current["Assigned Tech 2"] ?? "") !== schedule.assignedTech2) fields["Assigned Tech 2"] = schedule.assignedTech2 || null;
...
async function loadTables(baseId, tables, key, filters = {}, options = {}) {
  return loadAirtableTableSet(
    tables,
    (name, tableId) => listRecords(baseId, tableId, key, filters[name]),
    {
      ...options,
      onWarning: ({ name, records, error }) => console.warn(JSON.stringify({
        event: "airtable_optional_table_unavailable",
        table: name,
...
function opwpMetrics(data, { briaRouteAllocation = null, sngInvoiceMetrics = null, subscriptionTruth = null, cancellationMetrics = null, churnRange = null } = {}) {
  ...
  const churnReasonReviews = data.churn.filter((record) => {
    ...
    const commentMissing = churnReasonRequiresComment(category) && !String(record.fields.Comment || "").trim();
    return isStatus(record.fields["Review Status"], "Needs Reason", "Needs Validation") || commentMissing;
  });
  ...
  const reviewTrackingRecords = data.reviewTracker ?? [];
  const reviewClientRecords = reviewTrackingRecords.filter((record) => !isStatus(record.fields["Client Type"], "Unmatched review"));
  const reviewTrackingRows = reviewTrackingRecords.map((record) => ({
    id: record.id,
    customer: record.fields["Client Name"] || "Unknown",
    clientType: record.fields["Client Type"] || "Unknown",
    customerStatus: record.fields["Customer Status"] || "Unknown",
    reviewStatus: record.fields["Google Review"] || "Not reviewed",
    reviewName: record.fields["Review Name"] || "",
    reviewAliases: record.fields["Google Review Aliases"] || "",
    lastCompleted: record.fields["Last Completed Job"] || "",
    completedJobs: number(record.fields["Completed Jobs"]),
    priority: record.fields["Follow-up Priority"] || "",
  })).sort((a, b) => {
    const priority = (value) => value === "Highlight - recent one-time" ? 0 : value === "Future automation" ? 1 : value === "Do not contact - past one-time" ? 2 : value === "Do not contact - reviewed" ? 3 : 4;
    return priority(a.priority) - priority(b.priority) || String(b.lastCompleted).localeCompare(String(a.lastCompleted)) || a.customer.localeCompare(b.customer);
  });
  ...
  return {
    ...
    googleReviewTrackedClients: reviewClientRecords.length,
    googleReviewCompleted: reviewClientRecords.filter((record) => isStatus(record.fields["Google Review"], "Reviewed")).length,
    googleReviewMissing: reviewClientRecords.filter((record) => isStatus(record.fields["Google Review"], "Not reviewed")).length,
    googleReviewUnmatched: reviewTrackingRecords.length - reviewClientRecords.length,
    googleReviewRecentOneTimeCount: reviewTrackingRecords.filter((record) => isStatus(record.fields["Follow-up Priority"], "Highlight - recent one-time")).length,
    googleReviewRows: reviewTrackingRows,
...
export async function getAirtableBusinessCockpit(options = {}) {
  ...
  try {
    if (env.DB) {
      ...
      if (cached?.payload) {
        let data = JSON.parse(cached.payload);
        if (!Array.isArray(data?.opwp?.reviewTracker)) {
          const refreshed = await refreshAirtableCockpitSnapshot(env);
          data = refreshed.data;
          cached.status = "success";
          cached.error = null;
          cached.captured_at = refreshed.capturedAt;
        }
        const cachedTimestamp = String(cached.captured_at);
        const parsedTimestamp = cachedTimestamp.endsWith("Z") ? cachedTimestamp : `${cachedTimestamp.replace(" ", "T")}Z`;
        const ageHours = Math.max(0, (Date.now() - Date.parse(parsedTimestamp)) / 3600000);
...
export async function loadAirtableCockpitSource(key, fallback = {}) {
  const recent = (field, days = 120) => `IS_AFTER({${field}},DATEADD(TODAY(),-${days},'days'))`;
  const [opwpResult, dogFoodResult] = await Promise.all([
    ...
    }, { optional: ["leads", "reviewTracker"], fallback: fallback.opwp }),
    ...
    }, { fallback: fallback.dogFood }),
  ]);
  return {
    data: { opwp: opwpResult.data, dogFood: dogFoodResult.data },
    warnings: [...opwpResult.warnings, ...dogFoodResult.warnings],
  };
}
...
export async function refreshAirtableCockpitSnapshot(env = runtimeEnv()) {
  ...
  if (env.DB) {
    const prior = await env.DB.prepare(
      `SELECT payload FROM airtable_cockpit_snapshots WHERE snapshot_key='business_cockpit'`
    ).first();
    try { fallback = prior?.payload ? JSON.parse(prior.payload) : {}; } catch { fallback = {}; }
  }
  const source = await loadAirtableCockpitSource(env.AIRTABLE_API_KEY, fallback);
  const data = source.data;
  const status = source.warnings.length ? "partial" : "success";
  const warning = source.warnings.join(" ").slice(0, 500) || null;
  const capturedAt = new Date().toISOString();
  if (env.DB) {
    await env.DB.prepare(
      `INSERT INTO airtable_cockpit_snapshots (snapshot_key,payload,status,error,captured_at,updated_at)
       VALUES ('business_cockpit',?,?,?,?,CURRENT_TIMESTAMP)
       ON CONFLICT(snapshot_key) DO UPDATE SET payload=excluded.payload,status=excluded.status,error=excluded.error,captured_at=excluded.captured_at,updated_at=CURRENT_TIMESTAMP`
    ).bind(JSON.stringify(data), status, warning, capturedAt).run();
  }
  return { data, capturedAt, status, warning };
}
...
export async function ensureFreshAirtableCockpitSnapshot(env = runtimeEnv(), maxAgeMinutes = 15) {
  ...
  try {
    ...
    return { ok: true, refreshed: true, capturedAt: refreshed.capturedAt, ageMinutes: 0, warning: refreshed.warning };
  } catch (error) {
...
export async function runScheduledAirtableCockpitRefresh(env) {
  ...
  try {
    ...
    await env.DB.prepare(`INSERT INTO system_sync_runs (id,sync_name,status,snapshot_date,records_processed,error,completed_at) VALUES (?,'airtable_cockpit',?,?,1,?,CURRENT_TIMESTAMP)`).bind(id, result.status, result.capturedAt.slice(0, 10), result.warning).run();
    console.log(JSON.stringify({ event: "airtable_cockpit_refresh", capturedAt: result.capturedAt, status: result.status, warning: result.warning }));
    return result;
  } catch (error) {
    if (env?.DB) {
