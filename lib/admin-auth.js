...
import { dedicatedSigningSecret, isProductionEnvironment } from "./app-config";
...
function sessionSecret(env) {
  return dedicatedSigningSecret("ADMIN_SESSION_SECRET", env);
}
...
export async function verifyAdminRequest(headers) {
  ...
  if (!isProductionEnvironment(env) && isLocal && env.ADMIN_DEV_BYPASS === "true") return { authorized: true, email: "local-admin@opwp", method: "development" };
