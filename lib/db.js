...
import { PARTIAL_QUOTE_FOLLOW_UP_DELAY_MINUTES } from "./quote-funnel.mjs";

export function getDb() {
  return getRuntimeEnv().DB ?? null;
}

export async function saveSubmission({ kind, source = "website", status = "new", body, funnelId = "", lifecycleStage = "" }) {
  const db = getDb();
  ...
  await db.prepare(
    `INSERT INTO submissions
      (id, kind, source, status, name, email, phone, zip, payload, funnel_id, lifecycle_stage, sng_sync_state, last_activity_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'not_attempted', CURRENT_TIMESTAMP)`
  ).bind(id, kind, source, status, name, email, phone, zip, JSON.stringify(body), funnelId || null, lifecycleStage || null).run();

  return { configured: true, id };
}

function sngEntityId(response) {
  return response?.client ?? response?.client_id ?? response?.id ?? response?.data?.client ?? response?.data?.client_id ?? null;
}
...
export async function markSubmissionSynced(id, response, ok) {
  ...
  await db.prepare(
    `UPDATE submissions
     SET sng_synced = ?, sng_response = ?, status = ?, sng_sync_state = ?,
         sng_entity_id = COALESCE(?, sng_entity_id),
         lifecycle_stage = CASE WHEN ? THEN 'converted' ELSE lifecycle_stage END,
         last_activity_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP
     WHERE id = ?`
  ).bind(ok ? 1 : 0, JSON.stringify(response), ok ? "synced" : "needs_attention", ok ? "succeeded" : "failed", sngEntityId(response), ok ? 1 : 0, id).run();
}

export async function upsertPartialQuoteSubmission({ body, funnelId, lifecycleStage }) {
  const db = getDb();
  const id = crypto.randomUUID();
  if (!db) return { configured: false, id, created: false };
  const name = body.name || null;
  const result = await db.prepare(
    `INSERT INTO submissions
      (id,kind,source,status,name,email,phone,zip,payload,funnel_id,lifecycle_stage,sng_sync_state,last_activity_at)
     VALUES (?,'partial_quote','website','follow_up_pending',?,?,?,?,?,?,?,'not_supported',CURRENT_TIMESTAMP)
     ON CONFLICT(funnel_id) WHERE kind='partial_quote' AND funnel_id IS NOT NULL DO UPDATE SET
       name=COALESCE(excluded.name,submissions.name), email=excluded.email, phone=excluded.phone,
       zip=excluded.zip, payload=excluded.payload, lifecycle_stage=excluded.lifecycle_stage,
       last_activity_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP`
  ).bind(id, name, body.email, body.phone, body.zip || null, JSON.stringify(body), funnelId, lifecycleStage).run();
  const row = await db.prepare(
    "SELECT id,status,lifecycle_stage FROM submissions WHERE kind='partial_quote' AND funnel_id=? LIMIT 1"
  ).bind(funnelId).first();
  return { configured: true, id: row?.id || id, created: Number(result.meta?.changes ?? 0) > 0 && row?.id === id, row };
}

export async function queuePartialQuoteFollowUp({ submissionId, phone, consentAt, delayMinutes = PARTIAL_QUOTE_FOLLOW_UP_DELAY_MINUTES }) {
  const db = getDb();
  if (!db) return { configured: false, status: "queued" };
  const id = crypto.randomUUID();
  const safeDelay = Math.min(Math.max(Number(delayMinutes) || PARTIAL_QUOTE_FOLLOW_UP_DELAY_MINUTES, 5), 1440);
  const consentText = "I agree to receive automated text messages from Ohio Pet Waste Pros about my requested quote and service follow-up at the number provided. Message frequency varies. Message & data rates may apply. Reply STOP to opt out.";
  await db.prepare(
    `INSERT INTO quote_follow_ups
      (id,submission_id,recipient,status,consent_text,consent_version,consent_at,scheduled_at)
     VALUES (?,?,?,'queued',?,'quote_service_sms_v1',?,datetime('now',?))
     ON CONFLICT(submission_id) DO NOTHING`
  ).bind(id, submissionId, phone, consentText, consentAt, `+${safeDelay} minutes`).run();
  const row = await db.prepare("SELECT id,status,scheduled_at FROM quote_follow_ups WHERE submission_id=?").bind(submissionId).first();
  if (row?.id) {
    await db.batch(["sng_lead", "customer_email", "customer_sms"].map((channel) => db.prepare(
      `INSERT INTO quote_follow_up_deliveries (id,follow_up_id,submission_id,channel,status,next_attempt_at)
       VALUES (?,?,?,?,'queued',datetime('now',?)) ON CONFLICT(follow_up_id,channel) DO NOTHING`
    ).bind(crypto.randomUUID(), row.id, submissionId, channel, `+${safeDelay} minutes`)));
  }
  return { configured: true, status: row?.status || "queued", id: row?.id || id, scheduledAt: row?.scheduled_at || null };
}

export async function cancelAbandonedQuoteFollowUp({ email, funnelId = "" }) {
  const db = getDb();
  if (!db) return false;
  const normalizedEmail = String(email || "").trim().toLowerCase();
  if (!normalizedEmail && !funnelId) return false;
  const match = "kind='partial_quote' AND status IN ('new','follow_up_pending') AND (funnel_id=? OR lower(email)=?)";
  await db.prepare(
    `UPDATE quote_follow_ups SET status='cancelled',cancelled_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP
     WHERE status IN ('queued','failed')
       AND submission_id IN (SELECT id FROM submissions WHERE ${match})`
  ).bind(funnelId || "", normalizedEmail).run();
  await db.prepare(
    `UPDATE quote_follow_up_deliveries SET status='cancelled',cancelled_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP
     WHERE status IN ('queued','failed') AND submission_id IN (SELECT id FROM submissions WHERE ${match})`
  ).bind(funnelId || "", normalizedEmail).run();
  return true;
}

export async function beginOnboardingSubmission({ body, funnelId }) {
  const db = getDb();
  const id = crypto.randomUUID();
  if (!db) return { configured: false, id, claimed: true, existing: null };
  const name = [body.first_name, body.last_name].filter(Boolean).join(" ") || null;
  const inserted = await db.prepare(
    `INSERT INTO submissions
      (id,kind,source,status,name,email,phone,zip,payload,funnel_id,lifecycle_stage,sng_sync_state,last_activity_at)
     VALUES (?,'onboarding','website','syncing',?,?,?,?,?,?,'onboarding_submitted','in_progress',CURRENT_TIMESTAMP)
     ON CONFLICT(funnel_id) WHERE kind='onboarding' AND funnel_id IS NOT NULL DO NOTHING`
  ).bind(id, name, body.email, body.cell_phone_number, body.zip_code, JSON.stringify(body), funnelId).run();
  if (Number(inserted.meta?.changes ?? 0) > 0) return { configured: true, id, claimed: true, existing: null };

  const existing = await db.prepare(
    "SELECT id,status,sng_synced,sng_response,sng_entity_id,updated_at FROM submissions WHERE kind='onboarding' AND funnel_id=? LIMIT 1"
  ).bind(funnelId).first();
  if (existing?.status === "synced" && Number(existing.sng_synced) === 1) return { configured: true, id: existing.id, claimed: false, completed: true, existing };
  const reclaimed = await db.prepare(
    `UPDATE submissions SET status='syncing', sng_sync_state='in_progress', payload=?,
       last_activity_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP
     WHERE id=? AND (status='needs_attention' OR (status='syncing' AND updated_at < datetime('now','-5 minutes')))`
  ).bind(JSON.stringify(body), existing?.id || "").run();
  return { configured: true, id: existing?.id || id, claimed: Number(reclaimed.meta?.changes ?? 0) > 0, completed: false, existing };
}
...
export async function convertAbandonedQuote({ email, phone, funnelId = "", sngEntityId: entityId = null }) {
  const db = getDb();
  if (!db) return false;
  const normalizedEmail = String(email || "").trim().toLowerCase();
  if (!normalizedEmail || String(phone || "").replace(/\D/g, "").length < 10) return false;
  const match = "kind='partial_quote' AND status IN ('new','follow_up_pending') AND (funnel_id=? OR lower(email)=?)";
  await db.batch([
    db.prepare(
      `UPDATE submission_notifications SET status='cancelled',cancelled_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP
       WHERE notification_type='partial_quote' AND status IN ('queued','failed')
         AND submission_id IN (SELECT id FROM submissions WHERE ${match})`
    ).bind(funnelId || "", normalizedEmail),
    db.prepare(
      `UPDATE submissions SET status='converted', lifecycle_stage='converted',
         sng_entity_id=COALESCE(?,sng_entity_id), last_activity_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP
       WHERE ${match}`
    ).bind(entityId, funnelId || "", normalizedEmail),
    db.prepare(
      `UPDATE quote_follow_ups SET status='cancelled',cancelled_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP
       WHERE status IN ('queued','failed')
         AND submission_id IN (SELECT id FROM submissions WHERE ${match})`
...
export async function listSubmissions(limit = 50) {
  ...
  const result = await db.prepare(
    `SELECT id, kind, source, status, name, email, phone, zip, sng_synced,
            funnel_id, lifecycle_stage, sng_sync_state, sng_entity_id,
...
export async function listQuoteFunnel(limit = 200) {
  const db = getDb();
  if (!db) return { configured: false, rows: [] };
  const result = await db.prepare(
    `SELECT s.id,s.kind,s.status,s.name,s.email,s.phone,s.zip,s.funnel_id,s.payload,
            s.lifecycle_stage,s.sng_sync_state,s.sng_entity_id,s.last_activity_at,
            s.created_at,s.updated_at,
            GROUP_CONCAT(n.notification_type || ':' || n.status, ', ') notifications,
            (SELECT GROUP_CONCAT(d.channel || ':' || d.status, ', ') FROM quote_follow_up_deliveries d WHERE d.submission_id=s.id) followup_deliveries,
            (SELECT q.status FROM quote_follow_ups q WHERE q.submission_id=s.id LIMIT 1) sms_followup
     FROM submissions s
     LEFT JOIN submission_notifications n ON n.submission_id=s.id
     WHERE s.kind IN ('partial_quote','question','waitlist','onboarding')
     GROUP BY s.id
