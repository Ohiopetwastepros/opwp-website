...
export async function convertAbandonedQuote({ email, phone }) {
  const db = getDb();
  if (!db) return false;
  const normalizedEmail = String(email || "").trim().toLowerCase();
  if (!normalizedEmail || String(phone || "").replace(/\D/g, "").length < 10) return false;
  await db.prepare(
    `UPDATE submissions SET status='converted', updated_at=CURRENT_TIMESTAMP
     WHERE kind='partial_quote' AND status IN ('new','follow_up_pending')
       AND lower(email)=?`
