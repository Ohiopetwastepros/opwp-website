...
async function loadPlan(db, planId) {
  const [locationRows, taskRows] = await Promise.all([
    ...
    db.prepare(`SELECT t.id,t.location_id,t.task_type,t.status,t.customer_display_name,t.estimated_minutes,t.placement_note,t.product_summary,
      ...
      (SELECT COUNT(*) FROM route_partner_notification_outbox n WHERE n.task_id=t.id AND n.notification_type='on_the_way' AND n.status IN ('queued','sent')) AS notice_count
...
async function ownedContext(db, auth, body) {
  const row = await db.prepare(
    `SELECT s.id AS shift_id,s.status AS shift_status,s.starting_mileage,s.route_plan_id,s.technician_member_id,p.status AS plan_status
     FROM route_partner_field_shifts s JOIN route_partner_route_plans p ON p.id=s.route_plan_id
...
export async function applyFieldAction(db, auth, body) {
  ...
  if (action === "confirm_load") {
    const context = await ownedContext(db, auth, body);
    if (context.shift_status !== "pending_load") throw new Error("This departure check has already been completed.");
    const startingMileage = numberOrNull(body.startingMileage);
    if (startingMileage !== null && startingMileage < 0) throw new Error("Starting mileage cannot be negative.");
    ...
    statements.push(db.prepare("UPDATE route_partner_field_shifts SET status='ready',starting_mileage=?,checked_in_at=COALESCE(checked_in_at,CURRENT_TIMESTAMP),updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(startingMileage, context.shift_id));
  ...
  if (action === "start_task") {
    ...
    if (task.arrival_status !== "arrived") throw new Error("Record your arrival before starting this task.");
    if (!["scheduled", "ready"].includes(task.status)) throw new Error("This task cannot be started from its current status.");
    await db.batch([
  ...
  if (action === "on_the_way") {
    const { context, task } = await ownedTask(db, auth, body);
    if (context.shift_status !== "in_progress") throw new Error("Start the route before queuing a customer notice.");
    if (task.task_type !== "dog_food") throw new Error("On-the-way notices are only sent for food deliveries.");
    if (TERMINAL_TASKS.has(task.status)) throw new Error("This delivery is already closed.");
    const lead = Number(body.leadMinutes);
    if (!LEAD_OPTIONS.has(lead)) throw new Error("Select a valid notice time.");
    const priorNotice = await db.prepare("SELECT id FROM route_partner_notification_outbox WHERE task_id=? AND notification_type='on_the_way' AND status IN ('queued','sent') LIMIT 1").bind(task.id).first();
    if (priorNotice) throw new Error("An on-the-way notice is already queued for this delivery.");
    await db.batch([
  ...
  if (action === "complete_task") {
    const { context, task } = await ownedTask(db, auth, body);
    if (context.shift_status !== "in_progress") throw new Error("The route must be active before completing a task.");
    if (task.status !== "in_progress") throw new Error("Start this task before completing it.");
  ...
  if (action === "fail_task") {
    const { context, task } = await ownedTask(db, auth, body);
    if (context.shift_status !== "in_progress") throw new Error("The route must be active before reporting an exception.");
    if (TERMINAL_TASKS.has(task.status)) throw new Error("This task is already closed.");
  ...
  if (action === "start_break" || action === "end_break") {
    const context = await ownedContext(db, auth, body);
    if (context.shift_status !== "in_progress") throw new Error("Breaks can only be recorded during an active route.");
  ...
  if (action === "complete_route") {
    const context = await ownedContext(db, auth, body);
    if (context.shift_status !== "in_progress") throw new Error("Only an active route can be completed.");
    const activeBreak = await db.prepare("SELECT id FROM route_partner_field_breaks WHERE shift_id=? AND ended_at IS NULL LIMIT 1").bind(context.shift_id).first();
    if (activeBreak) throw new Error("End the active break before completing the route.");
    ...
    for (const item of inventory.results || []) {
      const maximumReturn = Math.max(0, Number(item.loaded_quantity) - Number(item.delivered_quantity));
      if ((returned.get(String(item.product_id)) ?? maximumReturn) > maximumReturn) throw new Error("Returned inventory cannot exceed the undelivered vehicle quantity.");
    }
    const endingMileage = numberOrNull(body.endingMileage);
    if (endingMileage !== null && endingMileage < 0) throw new Error("Ending mileage cannot be negative.");
    if (context.starting_mileage !== null && endingMileage !== null && endingMileage < Number(context.starting_mileage)) throw new Error("Ending mileage cannot be lower than starting mileage.");
    ...
    statements.push(db.prepare("UPDATE route_partner_field_shifts SET status='completed',ending_mileage=?,completed_at=CURRENT_TIMESTAMP,technician_notes=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(endingMileage, String(body.notes || "").slice(0, 1000) || null, context.shift_id));
